...
const StudentDetail = ({ params }: { params: { studentId: string } }) => {
  ...
  const [courseData, setCourseData] = useState<CoursSelection | any>();
  ...
  const getStudentData = async () => {
    const response: ServerResponse = await GetStudentById(params.studentId);
    ...
    if (response.status === HttpStatusCode.Ok) {
      ...
      setCourseData(student.educationalClasses.courseSelection);
      ...
      const sessionRes = await getSessionByClassId(
        student?.educationalClasses?._id || ""
      );
      setSessionData(sessionRes.data.data);
      const allCourseRes = await getAllCourse();
      setAllCourseData(allCourseRes.data.data);
    } else {
  ...
  const PersonalSubmitService = async (data: any) => {
    ...
    const res = await EditTeacherById(params.studentId, studentData);
