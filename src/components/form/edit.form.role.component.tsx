...
import style from "./edit.form.role.module.css";
...
import { getRoleById } from "@/services/role/role.service";
...
export default function EditFormRole({
  roleId,
  fieldsIsDisabled,
}: {
  roleId: string;
  ...
  const getRoleToEdit = async () => {
    const { data, status } = await getRoleById(roleId);
    if (status == HttpStatusCode.Ok) {
      const roleData: RoleTypeToInsert = data.data;
      setValue("roleName", roleData?.name);
      setValue("roleDescription", roleData?.description);
      setValue("roleAlias", roleData?.alias);
      setValue("roleColor", roleData?.color);
      setValue("_id", roleData?._id);
      initKeysWordsGroupsFromRoleToEdit(roleData?.groups);
      initKeysWordsPrivilegeFromRoleToEdit(roleData?.privileges);
    }
  };

  const initKeysWordsGroupsFromRoleToEdit = (groupFromRole: string[]) => {
    setKeysWordsGroups(groupFromRole);
  };

  const initKeysWordsPrivilegeFromRoleToEdit = async (
    privilegesFromRole: string[]
  ) => {
    setKeysWordsPrivileges(privilegesFromRole);
  };

  const initKeysWordsCheckAll = async () => {
    if (tabPrivilegesToDisplay && tabPrivilegesToDisplay.length > 0) {
      const tabWithCheckAll: tabPrivilege[] = getTabWithCheckAll();
      const tabCheck: string[] = [];
      tabWithCheckAll.map((item) => tabCheck.push(item.label));
      setKeysWordsAll(tabCheck);
    }
  };

  const getTabWithCheckAll = () => {
    const tabWithCheckAll: tabPrivilege[] = tabPrivilegesToDisplay.filter(
      (choices: tabPrivilege, index: number) => {
        let checkedPrivilege: PrivilegeType[] = choices.privilege.filter(
          (privilege: PrivilegeType) =>
            keysWordsPrivileges.includes(privilege._id as string)
        );
        if (choices.privilege.length == checkedPrivilege.length) {
          return choices;
        }
      }
    );
    return tabWithCheckAll;
  };
  ...
  const handleCheckForGroups = (e: any) => {
    const id_selected_group = typeof e === "string" ? e : e.target.value;
  ...
  const handleCheckPrivileges = (res: string) => {
  ...
  useEffect(() => {
    initKeysWordsCheckAll();
  }, [tabPrivilegesToDisplay]);
  ...
  useEffect(() => {
    ...
    keysWordsPrivileges,
    getValues("groups"),
    getValues("privileges"),
  ]);

  return (
    <>
      {console.log(keysWordsPrivileges)}
      <div className={style.layout}>
        <span className={style.childTitle}>Informations</span>
        <div className={style.containerPage}>
            ...
            <div className={style.containeChexBoxGroup}>
              {groups.map((group, indexGroup) => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={keysWordsGroups.includes(group?._id)}
                    value={group._id}
          ...
          {keysWordsGroups.length > 0 && (
            <div className={style.layoutContainerTabPrivilege}>
              <div
                className={`${style.containerTabPrivilege} ${
                  fieldsIsDisabled && style.notClickable
                }`}
              >
